...
export class HeuristicLocalAdapter implements AICommandAdapter {
  ...
  private extractCommand(text: string, _context: SimulationContext): AIStructuredCommand | null {
    ...
    if (
      ...
      (text.includes('create') || text.includes('spawn') || text.includes('add') || text.includes('make') || text.includes('summon'))
    ) {
      const isMassive = text.includes('massive') || text.includes('supermassive') || text.includes('huge') || text.includes('giant');
      return {
    ...
    if (
      ...
      (text.includes('explode') && (text.includes('star') || text.includes('sun') || text.includes('universe'))) ||
      text.includes('detonate')
    ) {
      return {
        action: 'CREATE_SUPERNOVA',
        parameters: {
          power: text.includes('huge') || text.includes('giant') || text.includes('mega') ? 2.0 : 1.2
        },
        confidence: 0.98,
        isDestructive: true,
        explanation: 'Triggering high-energy stellar Supernova detonation and particle ionization shockwave.'
      };
    }
    ...
    if (
      ...
      (text.includes('all') || text.includes('center') || text.includes('synchronize') || text.includes('system'))
    ) {
      return {
        action: 'ALIGN_ORBITS',
        parameters: {
        ...
        confidence: 0.95,
        explanation: 'Re-calculating Keplerian orbital velocities and locking all celestial bodies into stable concentric orbits.'
      };
    }
    ...
    if (
      ...
      (text.includes('create') || text.includes('spawn') || text.includes('add') || text.includes('make')) &&
      !text.includes('orbit the center')
    ) {
      const color = this.extractColor(text) || '#00f5a0';
      const isGiant = text.includes('giant') || text.includes('huge') || text.includes('large') || text.includes('gas');
    ...
    if (text.includes('gravity') || text.includes('gravitational')) {
      const isIncrease = text.includes('increase') || text.includes('higher') || text.includes('stronger') || text.includes('more') || text.includes('double') || text.includes('heavy');
      const isDecrease = text.includes('decrease') || text.includes('lower') || text.includes('weaker') || text.includes('less') || text.includes('half') || text.includes('light');
    ...
    if (
      (text.includes('orbit') || text.includes('realign') || text.includes('align')) &&
      (text.includes('all') || text.includes('center') || text.includes('planets') || text.includes('system') || text.includes('synchronize'))
    ) {
      return {
        action: 'ALIGN_ORBITS',
        parameters: {
    ...
    if (text.includes('destroy') || text.includes('kill') || text.includes('delete') || text.includes('remove') || text.includes('clear')) {
      ...
      const isAll = text.includes('everything') || text.includes('all bodies') || text.includes('all entities');

      return {
        action: 'DESTROY_ENTITY',
        parameters: {
          targetType: isAllPlanets ? 'ALL_PLANETS' : isBlackHoles ? 'ALL_BLACK_HOLES' : isAsteroids ? 'ASTEROID' : isAll ? 'ALL' : 'PLANET',
    ...
    if (text.includes('resume') || text.includes('play') || text.includes('unpause') || text.includes('continue')) {
      return {
    ...
    if (text.includes('speed') || text.includes('time') || text.includes('fast') || text.includes('slow')) {
      let scale = 1.0;
      if (text.includes('0.25') || text.includes('quarter')) scale = 0.25;
      else if (text.includes('0.5') || text.includes('half') || text.includes('slow motion')) scale = 0.5;
      else if (text.includes('2x') || text.includes('double') || text.includes('fast forward')) scale = 2.0;
    ...
    if (text.includes('seed life') || text.includes('spawn organism') || text.includes('add organism') || text.includes('create life') || text.includes('make life')) {
      return {
        action: 'SEED_LIFE',
    ...
    if (text.includes('energy') && (text.includes('bloom') || text.includes('burst') || text.includes('spawn') || text.includes('create') || text.includes('add'))) {
      return {
    ...
    if (text.includes('preset') || text.includes('solar system') || text.includes('binary') || text.includes('galaxy') || text.includes('lattice')) {
      let preset = 'SOLAR_SYSTEM';
      if (text.includes('binary') || text.includes('nebula')) preset = 'BINARY_STARS_NEBULA';
      else if (text.includes('black hole') || text.includes('accretion')) preset = 'BLACK_HOLE_ACCRETION';
      else if (text.includes('chaos') || text.includes('galaxy')) preset = 'CHAOS_GALAXY';
      else if (text.includes('lattice') || text.includes('classic') || text.includes('sphere')) preset = 'AETHERIA_LATTICE';
  ...
  private extractColor(text: string): string | null {
    ...
    if (text.includes('green') || text.includes('emerald') || text.includes('lime')) return '#00f5a0';
    if (text.includes('red') || text.includes('ruby') || text.includes('crimson')) return '#ff0844';
    if (text.includes('gold') || text.includes('yellow') || text.includes('amber')) return '#fee140';
    if (text.includes('purple') || text.includes('violet') || text.includes('magenta')) return '#c084fc';
  ...
  private extractPaletteIndex(text: string): number {
    if (text.includes('ultraviolet') || text.includes('deep purple') || text.includes('theme 0')) return 0;
    if (text.includes('cyan') || text.includes('electric') || text.includes('blue') || text.includes('theme 1')) return 1;
    if (text.includes('emerald') || text.includes('aurora') || text.includes('green') || text.includes('theme 2')) return 2;
    if (text.includes('supernova') || text.includes('crimson') || text.includes('red') || text.includes('theme 3')) return 3;
    if (text.includes('solar') || text.includes('gold') || text.includes('amber') || text.includes('yellow') || text.includes('theme 4')) return 4;
